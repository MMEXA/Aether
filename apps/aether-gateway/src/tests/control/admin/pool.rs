...
async fn gateway_batch_updates_shared_pool_key_configuration() {
    ...
    state
        .runtime_state()
        .kv_set(
            "upstream_models_provider:provider-openai",
            serde_json::to_string(&json!([{"id": "stale-provider-model"}]))
                .expect("provider model cache should serialize"),
            None,
        )
        .await
        .expect("provider model cache should seed");
    ...
    assert_eq!(
        state
            .runtime_state()
            .kv_get("upstream_models_provider:provider-openai")
            .await
            .expect("provider model cache should remain readable"),
        None,
        "provider key mutations must invalidate the provider-level model cache"
    );
